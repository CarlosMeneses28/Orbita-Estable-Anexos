...
// Reemplazar String de Sming con std::string
...
protected:
    static void toHex(u32 data, char* output, int len) {
		...
		ESP_LOGW("", "Input data: 0x%X (%u), len=%d", data, data, len);
        ...
        if (len == 6) {
            snprintf(output, 7, "%X%X%X%X%X%X", b2, b1, b4, b3, b6, b5);
        } else if (len == 4) {
            snprintf(output, 5, "%X%X%X%X", b2, b1, b4, b3);
        } else if (len == 2) {
            snprintf(output, 3, "%X%X", b2, b1);
        }
    }
...
public:
    ...
    String toString() const override {
        char ret[11];
        snprintf(ret, sizeof(ret), "=%s\r", mData);
        return String(ret);
    }
};
...
public:
    ...
    void setVersion(u8 major, u8 minor, u8 micro, u8 patch) {
        snprintf(mVersion, sizeof(mVersion), "%X%X%X%X00", major, minor, micro, patch);
    }

    String toString() const override {
        char ret[11];
        snprintf(ret, sizeof(ret), "=%s\r", mVersion);
        return String(ret);
    }
};
...
public:
    ...
    String toString() const override {
        char ret[11];
        snprintf(ret, sizeof(ret), "!%X\r", (int) mCode);
        return String(ret);
    }
};
...
public:
    ...
    String toString() const override {
        char ret[11];
        snprintf(ret, sizeof(ret), "=%X%X%X\r", b0, b1, b2);
        return String(ret);
    }
};
...
public:
    ...
    String toString() const override {
        char ret[12];  // Un byte extra para seguridad
        snprintf(ret, sizeof(ret), "=%X%X%X000\r", b0, b1, b2);
        return String(ret);
    }
};
...
protected:
    ...
    static u32 charToHex(char c) {
        ...
        case 'F':
            return 0x0F;
        default:
            ESP_LOGE("Commands", "charToHex: Bad char %c", c);
            return 0x00;
        }
    }
    ...
    static int parseHexLong(const char* data, int len) {
		...
		ESP_LOGW("", "OUTPUT data: 0x%X (%u), len=%d", ret, ret, len);
        return ret;
    }
    ...
    bool Parse(const char* data, int len) {
        ...
        if (len == MSG_SIZE) {
            if (data[0] == PULSE_PKT_START && sSeq != data[1]) {
                ...
                if (mDuration > 10000) {
                    ESP_LOGW("Commands", "Pulse guide parse: Too long pulse, limit to 10 secs.");
                    mDuration = 10000;
                }
                mRate = ((float)data[7])/10.0f;
                if (mRate > 10.0f) {
                    ESP_LOGW("Commands", "Pulse guide parse: Too fast rate pulse, limit to <x10 sideral.");
                    mRate = 10.0f;
                }else if (mRate < 0.1f) {
                    ESP_LOGW("Commands", "Pulse guide parse: Too slow rate pulse, limit to >x0.1 sideral.");
                    mRate = 0.1f;
                }
                ESP_LOGI("Commands", "Pulse guide parse: mAxis = %d, mDir = %d, mDuration = %d, mRate(x10) = %d",
                        (int)mAxis, (int)mDir, mDuration, (int)(mRate*10.0f));
                ret = true;
            }
        } else {
            ESP_LOGE("Commands", "Bad message size %d", len);
        }
        return ret;
    }
};
        ...
        if (len == MSG_SIZE) {
            if (data[0] == ':') {
                ...
                if (header == 'E') {
                    ...
                    ESP_LOGI("Commands", "SetPositionCommand: Parse => Axis: %d, Pos: %X", (int)mAxis, mPosition);
                    mInit = true;
                    ret = true;
                }
            }
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'S') {
					mAxis = parseAxis(data[2]);
					mPosition = parseHexLong(data + 3, 6);
					ESP_LOGE("Commands", "SetGotoTarget: Parse => Axis: %d, Pos: %X",
							(int)mAxis, mPosition);
					mInit = true;
					ret = true;
				}
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'H') {
					...
					ESP_LOGI("Commands", "SetGotoTargetIncrement: 0X%s: Parse => Axis: %d, Increment: %d",
							data + 3, (int)mAxis, mIncrement);
					mInit = true;
					ret = true;
				}
			}
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'M') {
					mAxis = parseAxis(data[2]);
					mIncrement = parseHexLong(data + 3, 6);
					ESP_LOGI("Commands", "SetBreakPointIncrement: Parse => Axis: %d, Increment: %d",
							(int)mAxis, mIncrement);
					mInit = true;
					ret = true;
				}
			}
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'I') {
					...
					ESP_LOGI("Commands", "SetStepPeriod: Parse => Axis: %d, Pos: %X",
							(int)mAxis, mPeriod);
					mInit = true;
					ret = true;
				}
	...
	bool Parse(const char* data, int len) {
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'F') {
					mAxis = parseAxis(data[2]);
					ESP_LOGI("Commands", "InitializationDone: Parse => Axis: %d", (int)mAxis);
					mInit = true;
					ret = true;
				}
			}
	...
	bool Parse(const char* data, int len) {
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'O') {
					mAxis = parseAxis(data[2]);
					mActive = parseHexUlong(data + 3, 1) != 0;
					ESP_LOGI("Commands", "SetAuxSwitch: Parse => Axis: %d, Active: %d", (int)mAxis, mActive);
					mInit = true;
					ret = true;
				}
			}
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'P') {
					...
					ESP_LOGI("Commands", "SetAutoGuideSpeed: Parse => Axis: %d, val: %d, speed: %d", (int)mAxis, val, mSpeed);
					mInit = true;
					ret = true;
				}
			}
	...
	bool Parse(const char* data, int len) {
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'J') {
					mAxis = parseAxis(data[2]);
					ESP_LOGI("Commands", "StartMotion: Parse => Axis: %d", (int)mAxis);
					mInit = true;
					ret = true;
				}
			}
	...
	bool Parse(const char* data, int len) {
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'K') {
					mAxis = parseAxis(data[2]);
					ESP_LOGI("Commands", "StopMotion: Parse => Axis: %d", (int)mAxis);
					mInit = true;
					ret = true;
				}
			}
	...
	bool Parse(const char* data, int len) {
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'L') {
					mAxis = parseAxis(data[2]);
					ESP_LOGI("Commands", "InstantStop: Parse => Axis: %d", (int)mAxis);
					mInit = true;
					ret = true;
				}
			}
...
public:
	...
	bool Parse(const char* data, int len) {
		bool ret = false;
		ESP_LOGI("Commands", "Try parse motion mode");
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				char header = data[1];
				if (header == 'G') {
					...
					case '3': {
						...
						break;
					default:
						ESP_LOGE("Commands", "SetMotionMode: Bad mode %c", mode);
					}
					...
					if (mInit) {
						ESP_LOGI("Commands", "SetMotionMode: Parse axis: %d, mode: %d, speed: %d, dir: %d", (int)mAxis, (int)mType, (int)mSpeed, (int)mDir);
					}
				}else{
					ESP_LOGE("Commands", "Bada header: [%c]", header);
				}
			}else{
				ESP_LOGE("Commands", "Bad data[0]: [%c]", data[0]);
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		}
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'e') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetVersion: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'a') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetCountsPerRev: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'b') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetTimerFreq: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'h') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetGotoTargetPosition: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'i') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetStepPeriod: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'j') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetPosition: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'f') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetStatus: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'g') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetHighSpeedRatio: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'D') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetSideralPeriod: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 's') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetPecPeriod: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'd') {
					...
					mInit = true;
					ret = true;
					ESP_LOGI("Commands", "GetAxisPosition: Parse axis: %d", (int)mAxis);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
		...
		if (len == MSG_SIZE) {
			if (data[0] == ':') {
				...
				if (header == 'q') {
					...
					ret = true;
					ESP_LOGI("Commands", "GetExtended: Parse axis: %d, id: %d", (int)mAxis, id);
				}
			}
		} else {
			ESP_LOGE("Commands", "Bad message size %d", len);
